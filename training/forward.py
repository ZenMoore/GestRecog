...
'''
# idea: 欠拟合：修改一圈激活函数后，分类正确率变成了89%
# choice for function:

    Optimizer= Adam
    Regularizer= L2
    Learning_rate_decay= cosine_decay_restarts
    Initializer_weights= He_initializer
    Initializer_bias= He_initializer
...
def get_weight_variable(shape, regularizer):
    weights = tf.get_variable('weights', shape= shape, initializer= tf.contrib.layers.variance_scaling_initializer()) # todo 可以尝试更换
...
def inference(input_tensor, regularizer):
    biases = tf.get_variable('biases', [48], initializer= tf.contrib.layers.variance_scaling_initializer())
    input_tensor_4dims = add_one_dimension(input_tensor)

    with tf.variable_scope('layer_conv01'):

        conv01_filter_weight = tf.get_variable('filter_weight', [1, 3, 1, 12], initializer= tf.contrib.layers.variance_scaling_initializer()) # todo 可以尝试更换
        conv01_biases = tf.get_variable('bias', [12], initializer=tf.contrib.layers.variance_scaling_initializer())
    ...
    with tf.variable_scope('layer_conv02'):
        conv02_filter_weight = tf.get_variable('filter_weight', [1, 3, 12, 24], initializer= tf.contrib.layers.variance_scaling_initializer()) # todo 可以尝试更换
        conv02_biases = tf.get_variable('bias', [24], initializer=tf.contrib.layers.variance_scaling_initializer())
    ...
    with tf.variable_scope('layer_conv03'):
        conv03_filter_weight = tf.get_variable('filter_weight', [1, 3, 24, 48], initializer= tf.contrib.layers.variance_scaling_initializer()) # todo 可以尝试更换
        conv03_biases = tf.get_variable('bias', [48], initializer=tf.contrib.layers.variance_scaling_initializer())
    ...
    with tf.variable_scope('layer_conv04'):
        conv04_filter_weight = tf.get_variable('filter_weight', [1, 3, 48, 48], initializer= tf.contrib.layers.variance_scaling_initializer()) # todo 可以尝试更换
        conv04_biases = tf.get_variable('bias', [48], initializer=tf.contrib.layers.variance_scaling_initializer())
    ...
    with tf.variable_scope('gru_layers'):
        ...
        gru = tf.nn.rnn_cell.GRUCell(GRU_HIDDEN_SIZE, activation= tf.nn.tanh)
    ...
    with tf.variable_scope('fully_connected'):
        ...
        fc_weight = tf.get_variable('fc_weight', [GRU_HIDDEN_SIZE, OUTPUT_NODE], initializer= tf.contrib.layers.variance_scaling_initializer())

        if regularizer != None:
            tf.add_to_collection('losses', regularizer(fc_weight))
        fc_bias = tf.get_variable('bias', [OUTPUT_NODE], initializer= tf.contrib.layers.variance_scaling_initializer())
